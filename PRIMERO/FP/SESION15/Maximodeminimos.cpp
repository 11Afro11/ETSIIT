...
public:
	...
	/*
	SecuenciaEnteros (SecuenciaEnteros entero) 	
	{		
		total_utilizados = 0;
	
		int longitud_la_palabra = entero.TotalUtilizados();
			
		if (longitud_la_palabra <= TAMANIO) {
			
			for (int i=0; i<longitud_la_palabra; i++)
				vector_privado[i]= entero[i];
...
public:
	...
	*/
	...
	int MaximoDeMinimos(){
		...
		SecuenciaEnteros secuencia_minimos;

		for(int i = 0; i <filas_utilizadas; i++){
			secuencia_minimos.Aniade(Fila(i).Minimo());
		}

		return secuencia_minimos.Maximo();
	}
...
int main (void)
{
	...
	TablaRectangularEnteros matriz (num_columnas);
	...
	for (int f=0; f<num_filas; f++) {
		...
		int elemento;
		int contador = 0;
		while (contador < num_columnas){
			cin >> elemento;
			una_cadena.Aniade(elemento);
			contador++;
		}
		

		cout << "cadena = ";
		for(int i = 0; i < una_cadena.TotalUtilizados(); i++){
			cout << una_cadena.Elemento(i);
		}
		cout << endl << endl; 
		
		// Crear una secuencia a partir de la cadena leida
		SecuenciaEnteros una_fila = una_cadena; 
	...
	cout << "\n\nEl maximo de los minimos de la funcion es: "<<matriz.MaximoDeMinimos();
